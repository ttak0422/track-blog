...
site:
	...
	nix develop "$(TRACK_SRC)" --command bash -euo pipefail -c 'make -C "$(TRACK_SRC)" site SITE_VAULT="$(CURDIR)" SITE_OUT="$(SITE_OUT_PATH)" SITE_CACHE="$(SITE_CACHE_PATH)" SITE_BASE="$(SITE_BASE)" SITE_ORIGIN="$(SITE_ORIGIN)"'
